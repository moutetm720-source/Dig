/**
 * verify-hermes.mjs — Tests E2E du moteur HERMES v5 (tests uniquement).
 *
 * Prérequis : serveur démarré avec un fournisseur IA RÉEL (GEMINI_API_KEY ou
 * HERMES_OPENAI_BASE_URL + modèle), base seedée par scripts/start-test-pg.mjs.
 *
 * ⚠️ Le mode « mock » (ancien mode TEST) a été SUPPRIMÉ du moteur : les
 * vérifications qui exigent une interprétation en langage naturel sont
 * ignorées (SKIP) si aucun fournisseur réel n'est configuré, plutôt que
 * d'être validées par une réponse simulée.
 *
...
// Fournisseur IA RÉEL disponible ? (détecté sur /api/hermes/status)
let llmReady = false;
function checkLLM(name, cond, detail = '') {
  if (!llmReady) {
    console.log(`SKIP ⏭️   ${name}  [aucun fournisseur IA réel configuré — le mock a été supprimé]`);
    return;
  }
...
(async () => {
  ...
  check('status → 200 + moteur v5 (pool multi-fournisseurs RÉELS)', r.status === 200 && String(r.json.engine).startsWith('hermes-core-v5'), JSON.stringify(r.json).slice(0, 140));
  check('status → realOnly=true (aucun fournisseur mock/simulé)', r.json.realOnly === true, `realOnly=${r.json.realOnly}`);
  check('status → AUCUN fournisseur mock dans le pool', !(r.json.providerPool || []).some(e => e.kind === 'mock' || e.name === 'mock-env'), JSON.stringify((r.json.providerPool || []).map(e => e.name)));
  llmReady = r.status === 200 && r.json.status === 'active' && !/^mock/i.test(String(r.json.provider || ''));
  if (!llmReady) console.log('   ⚠️  Aucun fournisseur IA réel : les tests d\'interprétation seront SKIPPÉS (jamais simulés).');
  ...
  checkLLM('BOUTIQUE RÉELLEMENT MODIFIÉE : prod-test-1 à 39 €', price1 === 39, `prix=${price1}`);

  // H2 : suppression → confirmation obligatoire
  r = await aiChat('Supprime prod-test-3');
  checkLLM('suppression → demande de confirmation (pas d\u2019exécution)', r.status === 200 && r.json.pendingConfirmation && r.json.pendingConfirmation.tool === 'catalog_delete', JSON.stringify(r.json.pendingConfirmation || {}).slice(0, 120));
  store = await req('GET', '/api/store');
  prods = (store.json || []).filter(x => x.key === 'dpf_app_v2_products');
  checkLLM('produit TOUTESFOIS PRÉSENT (non supprimé sans confirmation)', prods[0]?.value?.some(p => p.id === 'prod-test-3'));
  ...
  checkLLM('confirmation → exécution de la suppression', r.status === 200 && r.json.confirmed === true, `HTTP ${r.status}`);
  store = await req('GET', '/api/store');
  prods = (store.json || []).filter(x => x.key === 'dpf_app_v2_products');
  checkLLM('produit RÉELLEMENT SUPPRIMÉ après confirmation', !prods[0]?.value?.some(p => p.id === 'prod-test-3'));

  // H4 : création de produit réel (draft)
  r = await aiChat('Crée un produit « Test Kit Hermes » à 19.90 €');
  checkLLM('création → outil catalog_create exécuté', r.status === 200 && r.json.steps.some(s => s.tool === 'catalog_create' && s.status === 'ok'), JSON.stringify(r.json.steps || []).slice(0, 140));
  store = await req('GET', '/api/store');
  prods = (store.json || []).filter(x => x.key === 'dpf_app_v2_products');
  const newProd = prods[0]?.value?.find(p => p.id?.startsWith('prod-hermes-'));
  checkLLM('produit RÉELLEMENT CRÉÉ (statut draft, prix 19.90)', newProd && newProd.status === 'draft' && newProd.pricing?.recommendedPrice === 19.9, newProd ? `${newProd.id} ${newProd.status} ${newProd.pricing?.recommendedPrice}` : 'introuvable');

  console.log('\n═══ HERMES V4 — RGPD : aucune PII au LLM ═══');

  r = await aiChat('Fais un audit global de la fabrique');
  const auditResponse = r.json?.response || '';
  checkLLM('audit exécuté via outil réel', r.status === 200 && r.json.steps.some(s => s.tool === 'audit_system' && s.status === 'ok'));
  checkLLM('réponse SANS e-mail client (PII filtrée)', !auditResponse.includes('jeanne.dupont@example.com') && !auditResponse.includes('paul.martin@example.com'));
  checkLLM('réponse SANS noms de clients', !auditResponse.includes('Jeanne Dupont') && !auditResponse.includes('Paul Martin'));

  // Journal d'audit des actions
  r = await req('GET', '/api/hermes/activity', A);
  const toolsLogged = (r.json.activity || []).map(a => a.tool);
  checkLLM('journal d\u2019audit : actions outillées tracées', r.status === 200 && toolsLogged.includes('catalog_set_price') && toolsLogged.includes('catalog_delete') && toolsLogged.includes('catalog_create'), toolsLogged.slice(0, 6).join(','));
  ...
  checkLLM('free_tier_lookup → skill exécutée', r.status === 200 && kbStep && kbStep.status === 'ok', JSON.stringify(kbStep || {}).slice(0, 120));
  checkLLM('réponse KB → services réels listés (Render/Vercel/Netlify…)', (r.json.response || '').includes('free-for.dev') || /Render|Vercel|Netlify|Cloudflare/i.test(r.json.response || ''), (r.json.response || '').slice(0, 100).replace(/\n/g, ' '));
  ...
  checkLLM('free_llm_lookup → skill exécutée', r.status === 200 && llmStep && llmStep.status === 'ok', JSON.stringify(llmStep || {}).slice(0, 120));
  checkLLM('réponse KB LLM → providers réels (Groq/OpenRouter/Gemini…)', /awesome-free-llm-apis|Groq|OpenRouter|Gemini|Mistral/i.test(r.json.response || ''), (r.json.response || '').slice(0, 100).replace(/\n/g, ' '));
  ...
  checkLLM('web_fetch → lecture de page réelle', r.status === 200 && fetchStep && (fetchStep.status === 'ok' || /timeout|indisponible|réseau/i.test(fetchStep.summary || r.json.response || '')), JSON.stringify(fetchStep || {}).slice(0, 120));
  ...
  checkLLM('web_search → exécutée OU échec réseau signalé honnêtement (sandbox)', r.status === 200 && searchHonest, JSON.stringify(searchStep || {}).slice(0, 120));
  checkLLM('web_search → jamais de résultat inventé en cas d\'échec', !(searchStep && searchStep.status !== 'ok' && /résultat.*trouvé|source :.*https/i.test(r.json.response || '')), (r.json.response || '').slice(0, 80).replace(/\n/g, ' '));

  console.log('\n═══ HERMES V4 — Cycle autonome & multi-agents ═══');

  r = await aiPost('/api/hermes/autonomous-loop', {});
  checkLLM('cycle autonome → insight réel (agent security_auditor)', r.status === 200 && r.json.success === true && typeof r.json.insight === 'string' && r.json.insight.length > 30, (r.json.insight || '').slice(0, 80).replace(/\n/g, ' '));
  ...
  checkLLM('chat \u00ab liste les repos \u00bb \u2192 skill repos_list ex\u00e9cut\u00e9e', r.status === 200 && rStep && rStep.status === 'ok', JSON.stringify(rStep || {}).slice(0, 120));
  checkLLM('repos_list \u2192 repos r\u00e9els du harvest (crewAI/shadcn seed\u00e9s)', /crewAI|shadcn/i.test(r.json.response || '') || /crewAI|shadcn/i.test(JSON.stringify(r.json.steps || [])), (r.json.response || '').slice(0, 80).replace(/\n/g, ' '));

  // R2 : repos_get \u2014 d\u00e9tail d\u2019un repo par nom
  r = await aiChat('d\u00e9tail du repo crewAI');
  rStep = (r.json.steps || []).find(s => s.tool === 'repos_get');
  checkLLM('chat \u00ab d\u00e9tail du repo crewAI \u00bb \u2192 skill repos_get (id crewAI)', r.status === 200 && rStep && rStep.status === 'ok' && /crewAI/i.test(JSON.stringify(rStep.args || rStep.summary || {})), JSON.stringify(rStep || {}).slice(0, 120));
  ...
  checkLLM('chat \u00ab veille github \u00bb \u2192 skill repos_harvest (ok OU \u00e9chec r\u00e9seau signal\u00e9 honn\u00eatement)', r.status === 200 && rStep && (rStep.status === 'ok' || /GitHub|r\u00e9seau|indispo|timeout|API|quota/i.test(rStep.summary || '')), JSON.stringify(rStep || {}).slice(0, 140));
  if (rStep && rStep.status === 'ok') {
    ...
    checkLLM('repos_harvest \u2192 repos R\u00c9ELLEMENT ajout\u00e9s au harvest (KV serveur, d\u00e9doublonn\u00e9s)', reposKv.length > 0 && Array.isArray(reposKv[0].value) && reposKv[0].value.length >= 2, reposKv[0] ? `${reposKv[0].value.length} repos` : 'KV absente');
  }
  ...
  checkLLM('chat \u00ab r\u00e9f\u00e9rentiels locaux \u00bb \u2192 skill reference_repos ex\u00e9cut\u00e9e', r.status === 200 && rStep && rStep.status === 'ok', JSON.stringify(rStep || {}).slice(0, 140));
  ...
  checkLLM('chat \u00ab liens de la plateforme \u00bb \u2192 skill platform_links ex\u00e9cut\u00e9e', r.status === 200 && rStep && rStep.status === 'ok', JSON.stringify(rStep || {}).slice(0, 120));
  checkLLM('platform_links \u2192 sitemap.xml dans le r\u00e9sultat', /sitemap\.xml/.test(JSON.stringify(r.json.steps || [])), '');
  ...
  checkLLM('chat \u00ab vue globale \u00bb \u2192 skill platform_overview ex\u00e9cut\u00e9e', r.status === 200 && rStep && rStep.status === 'ok', JSON.stringify(rStep || {}).slice(0, 120));

  // R7 : /status enrichi (repos, autonomie)
  r = await req('GET', '/api/hermes/status');
  checkLLM('status \u2192 reposCount + autonomie + skills \u2265 45', r.status === 200 && typeof r.json.reposCount === 'number' && r.json.autonomy && typeof r.json.autonomy.enabled === 'boolean' && r.json.skillsCount >= 45, JSON.stringify({ repos: r.json.reposCount, skills: r.json.skillsCount, auto: r.json.autonomy }));
  ...
  checkLLM('cycle manuel \u2192 rapport r\u00e9el (donn\u00e9es, longueur > 60)', r.status === 200 && r.json.success === true && typeof r.json.report.report === 'string' && r.json.report.report.length > 60, (r.json.report?.report || '').slice(0, 100).replace(/\n/g, ' '));
  checkLLM('cycle \u2192 provider d\u00e9clar\u00e9 honn\u00eatement (mock/d\u00e9terministe)', /mock|d\u00e9terministe/i.test(r.json.report?.provider || ''), r.json.report?.provider || '');
  checkLLM('cycle \u2192 actions S\u00dcbRES uniquement (jamais publish/price/delete/dispatch/kv_set)', !(r.json.report?.actions || []).some(a => /^(publish_product|catalog_set_price|catalog_delete|catalog_update|channels_dispatch|kv_set|code_fix|providers_add|email_sequence_add)$/.test(a.tool)), JSON.stringify((r.json.report?.actions || []).map(a => a.tool)));
  ...
  r = await aiPost('/api/hermes/autonomous-loop', {});
  checkLLM('legacy /autonomous-loop \u2192 d\u00e9l\u00e8gue au nouveau cycle (insight = rapport r\u00e9el)', r.status === 200 && r.json.success === true && typeof r.json.insight === 'string' && r.json.insight.length > 30, (r.json.insight || '').slice(0, 80).replace(/\n/g, ' '));
  ...
  check('providers (auth) → 200 + AUCUN fournisseur mock dans le pool + aucune clé en clair', r.status === 200 && Array.isArray(r.json.pool) && !(r.json.pool || []).some(e => e.kind === 'mock' || e.name === 'mock-env') && (!llmReady || r.json.pool.length >= 1) && !/sk-|ghp_|api[_-]?key.{0,4}[\"']\s*:\s*[\"'][A-Za-z0-9]{12,}/i.test(r.text), `pool=${(r.json.pool || []).map(e => e.name).join(',')}`);
  ...
  const fallbackOk = r.status === 200 && !/^mock/i.test(String(r.json.provider || '')) && String(r.json.provider || '') !== 'aucun';
  checkLLM('chat RÉUSSIT via bascule automatique (cassé → fournisseur réel suivant) — jamais bloqué, jamais simulé', fallbackOk && r.json.steps.some(s => s.tool === 'audit_system' && s.status === 'ok'), `provider=${r.json?.provider}`);
  r = await req('GET', '/api/hermes/providers', A);
  const brokenStat = (r.json.pool || []).find(e => e.name === 'test-broken');
  checkLLM('fournisseur cassé → cooldown + erreur tracée (stats transparentes)', brokenStat && brokenStat.errors >= 1 && brokenStat.inCooldown === true, JSON.stringify(brokenStat || {}).slice(0, 120));
  ...
  checkLLM('retrait du fournisseur cassé (DELETE) → ok', r.status === 200 && r.json.removed === 'test-broken', `HTTP ${r.status}`);
  r = await req('DELETE', '/api/hermes/providers/ollama-local', A);
  checkLLM('retrait du fournisseur Ollama (DELETE) → ok', r.status === 200 && r.json.removed === 'ollama-local', `HTTP ${r.status}`);
  r = await req('GET', '/api/hermes/providers', A);
  check('pool final → uniquement des fournisseurs RÉELS (aucun mock-env)', r.status === 200 && !(r.json.pool || []).some(e => e.kind === 'mock' || e.name === 'mock-env'), (r.json.pool || []).map(e => e.name).join(','));

  console.log('\n═══ MODE 100 % RÉEL — IA, paiement et données ═══');

  r = await req('POST', '/api/checkout/demo-complete', { body: { serverOrderId: 'srv-0000000000' } });
  check('mode démo SUPPRIMÉ : /api/checkout/demo-complete → 410 Gone', r.status === 410, `HTTP ${r.status}`);

  r = await req('GET', '/api/checkout/config');
  check('checkout config → demoEnabled=false (paiement réel uniquement)', r.status === 200 && r.json.demoEnabled === false, JSON.stringify(r.json).slice(0, 120));

  r = await req('GET', '/api/hermes/skills');
  const realNames = (r.json.skills || []).map(s => s.name);
  check('skills « 100 % réel » disponibles (data_reality_audit + data_purge_demo)', realNames.includes('data_reality_audit') && realNames.includes('data_purge_demo'), `${realNames.length} skills`);

  r = await req('POST', '/api/hermes/config', { body: { provider: 'mock' }, auth: PASSCODE });
  check('POST /hermes/config provider=mock → 400 (mode test définitivement supprimé)', r.status === 400, `HTTP ${r.status}`);

  r = await req('POST', '/api/hermes/providers', { body: { name: 'mock-test', kind: 'mock' }, auth: PASSCODE });
  check('ajout d\'un fournisseur kind=mock → 400 (fournisseurs réels uniquement)', r.status === 400, `HTTP ${r.status}`);
