...
// 100 % RÉEL : plus d'approvals pré-remplies avec des statistiques inventées (ROAS, taux de conversion, tests A/B). Elles sont générées par de vraies analyses.
export const initialApprovals: ApprovalItem[] = [];

// 100 % RÉEL : plus de recommandations pré-remplies avec des chiffres inventés (CA, ROAS, conversion). Elles proviennent de l'analyse de vraies données.
export const initialRecommendations: Recommendation[] = [];

// 100 % RÉEL : les anciens journaux « seedés » (faux paiements Stripe, faux ROAS, faux tokens de livraison) ont été SUPPRIMÉS. Le journal ne contient que de vrais événements, produits par l'application.
export const initialSystemLogs: SystemLog[] = [];

// 100 % RÉEL : plus de tâches planifiées fictives. Les jobs listés sont ceux réellement exécutés par le serveur.
